...
// GTK4 basic_graphics_view handler implementation.
//
// The handler renders `basic_graphics_view.drawable` into a facade canvas
...
// clear if there's no bound basic_graphics_view or no drawable set.
void on_draw_func(GtkDrawingArea* /*area*/,
    ...
    if (h == nullptr) return;
    basic_graphics_view* gv = h->bound();
    ...
    // basic_image-surface-for-data wraps an existing memory buffer with the
...
graphics_view_handler<platform::linux_>::graphics_view_handler() {
    ...
    // pick up the current basic_graphics_view + drawable.
...
void graphics_view_handler<platform::linux_>::map_size(basic_graphics_view& gv) {
    bound_ = &gv;
...
void graphics_view_handler<platform::linux_>::map_draw_count(basic_graphics_view& gv) {
...
void graphics_view_handler<platform::linux_>::map_drawable(basic_graphics_view& gv) {
