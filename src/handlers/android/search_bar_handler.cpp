...
// Part of MPAPP. Android search_bar handler implementation.
...
void search_bar_handler<platform::android>::map_text(search_bar& s) {
    apply_text(s.text.get());
    s.text.changed.subscribe(text_slot_, text_cb_);
}
void search_bar_handler<platform::android>::map_placeholder(search_bar& s) {
