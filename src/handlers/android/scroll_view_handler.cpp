...
jobject child_jobject(view* v) {
    ...
    if (auto* tp = dynamic_cast<time_picker*>(v);        tp && tp->has_handler()) return tp->handler().native();
    if (auto* im = dynamic_cast<image*>(v);              im && im->has_handler()) return im->handler().native();
    return nullptr;
}
