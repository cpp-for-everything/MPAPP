...
// Part of MPAPP. WinUI 3 basic_slider handler implementation.
...
void slider_handler<platform::windows>::map_value(basic_slider& s) {
    bound_ = &s;
...
void slider_handler<platform::windows>::map_minimum(basic_slider& s) {
    apply_minimum(s.minimum.get());
    s.minimum.changed.subscribe(minimum_slot_, minimum_cb_);
}

void slider_handler<platform::windows>::map_maximum(basic_slider& s) {
