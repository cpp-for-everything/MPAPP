...
// Part of MPAPP. T-0011 follow-up — WinUI 3 entry handler implementation.
...
void entry_handler<platform::windows>::map_text(entry& e) {
    bound_ = &e;
...
void entry_handler<platform::windows>::map_placeholder(entry& e) {
    apply_placeholder(e.placeholder.get());
    e.placeholder.changed.subscribe(placeholder_slot_, placeholder_cb_);
}

void entry_handler<platform::windows>::map_is_read_only(entry& e) {
