...
// Part of MPAPP. WinUI 3 basic_tabbed_view handler implementation.
...
tabbed_view_handler<platform::windows>::tabbed_view_handler() {
    ...
    // tabs reflect the model's `tab_titles` — so hide the add basic_button and
...
void tabbed_view_handler<platform::windows>::map_tab_titles(basic_tabbed_view& t) {
    apply_tab_titles(t.tab_titles.get());
    t.tab_titles.changed.subscribe(tab_titles_slot_, tab_titles_cb_);
}

void tabbed_view_handler<platform::windows>::map_selected_index(basic_tabbed_view& t) {
