...
// Part of MPAPP. T-0011 — UIKit button handler implementation.
...
void button_handler<platform::ios>::map_text(button& b) {
    apply_text(b.text.get());
    b.text.changed.subscribe(text_slot_, text_cb_);
}

void button_handler<platform::ios>::map_clicked(button& b) {
    if (!native_) return;
    UIButton* btn = (__bridge UIButton*)native_;
