...
The Linux GTK4 demo screenshot is missing for the WSLg compositor-cache reason documented in `notes/linux-wslg-screenshot-quirk.md` — same quirk as T-0017 and T-0018. The msrdc proxy windows still show stale T-0018 content because `msrdc.exe` survives the `wsl --shutdown` mitigation, and we did not force-kill msrdc out of respect for other WSLg sessions in the same Windows session.
