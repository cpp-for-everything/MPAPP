...
## 2026-W20 — app-shell abstraction proposed

- [[ADR-0012-application-window-handler-abstraction]] (proposed) — Application / Window / Page / StackLayout / Grid extend the existing partial-specialisation widget-handler pattern; `mpapp::application` is single-instance per process on every platform; entry point is `mpapp::run<App>(argc, argv)` (no public-API macro); framework-owned `orientation` / `thickness` / `h_align` / `v_align` types. Filed in response to a user observation that `examples/windows_button_spike/main.cpp` carried ~25 raw `winrt::`/`mux::`/`muxc::`/`Mdd*` tokens in user-facing code, violating [[ADR-0006-interop-parity]] above the widget layer. Gated by [[T-0011-app-shell-abstraction]].
