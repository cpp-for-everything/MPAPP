...
- [[ADR-0012-application-window-handler-abstraction]] (proposed → **accepted** 2026-05-18) — Application / Window / Page / StackLayout / Grid extend the existing partial-specialisation widget-handler pattern; `mpapp::application` is single-instance per process on every platform; entry point is `mpapp::run<App>(argc, argv)` (no public-API macro); framework-owned `orientation` / `thickness` / `h_align` / `v_align` types. Filed in response to a user observation that `examples/windows_button_spike/main.cpp` carried ~25 raw `winrt::`/`mux::`/`muxc::`/`Mdd*` tokens in user-facing code, violating [[ADR-0006-interop-parity]] above the widget layer. **Closed by [[_Archive/T-0011-app-shell-abstraction|T-0011]]** with live verification on three runtime platforms: Windows (WinUI 3, Count: 0 → 7), Linux (GTK4 via WSLg, Count: 0 → 5), Android (NDK r26 emulator, Count: 0 → 7). macOS + iOS handlers code-complete pending an Apple host. Five components (Application / Window / StackLayout / Button / Label) shipped as `android-real` with three-platform-real bool flags. 126/126 tests still pass. Coverage report at `_Archive/T-0011-app-shell-abstraction/tests/coverage-report.md`.
