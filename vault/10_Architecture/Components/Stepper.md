...
| Layer | Class | Header |
|---|---|---|
| Surface — platform-agnostic, handler held by pointer | `mpapp::internal::basic_stepper` | [`include/mpapp/internal/basic_stepper.hpp`](../../../include/mpapp/internal/basic_stepper.hpp) |
| Wrapper — user-facing, embeds the platform handler by value | `mpapp::stepper` | [`include/mpapp/stepper.hpp`](../../../include/mpapp/stepper.hpp) |
...
mpapp::internal::basic_stepper w;
mpapp::stepper_handler<mpapp::platform::mock> h;
// h.map_<property>(w);  // exercise the mapper contract
```

The `mpapp::stepper_handler<Platform>` alias (template, defaults to `platform::current`) keeps `mpapp::stepper_handler<>` and `mpapp::stepper_handler<platform::mock>` valid spellings without naming `internal::`.
