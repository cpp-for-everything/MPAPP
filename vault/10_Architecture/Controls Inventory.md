...
**Total: 65 components.** **61 widgets at `android-real`** on Win / Linux / Android (every concrete widget; ContentView's frontmatter was reconciled to `android-real` to match its real handlers + this snapshot — T-0032 audit). Only the three abstract bases (View / Layout / Element) and the `Cell` abstract base remain terminal-mock by design — they don't have native primitives, only their subclasses do. ShapeView + GraphicsView v1 ship with per-platform native primitives; the unified canvas facade (Cairo / Skia compile-time selectable) remains v2 scope per [[ADR-0015-graphics-backend-dual]]. macOS + iOS pending Apple host.
