...
| [[Components/CarouselView\|CarouselView]] | `Handlers/CarouselView/` | mock (RFC-0013-era gap fill — `basic_carousel_view` surface: items_source + position + loop + is_swipe_enabled + peek_count + PositionChanged; scroll_to wraps/clamps per loop. Mock handler + tests. Per-platform real handlers — WinUI FlipView / GTK4 Adw.Carousel / Android ViewPager2 — are follow-ups, the standard mock → platform-real progression.) |
