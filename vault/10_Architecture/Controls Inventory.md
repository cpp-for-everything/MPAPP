...
| [[Components/CollectionView\|CollectionView]] | `Handlers/CollectionView/` | android-real (stable outer container — mux::Border / GtkScrolledWindow / FrameLayout — wraps an inner list-or-grid widget that swaps with `layout`: vertical_list=ListView/GtkListBox/ListView, vertical_grid=GridView/GtkFlowBox/GridView. SelectionMode mapping is uniform across inner types via the shared base API (mux::ListViewBase / GtkSelectionMode / android.widget.AbsListView). Multi-select event round-trip + item_tapped + selected_indices wired on all 3 platforms for both inner types. `typed_items` parallel surface for rendering arbitrary view*/cell* per item via ADR-0013 dispatch (Android swaps inner to ScrollView+LinearLayout in typed mode). horizontal_list / horizontal_grid degrade to vertical for v1; selection / layout enum ignored in typed mode for v1.) |
