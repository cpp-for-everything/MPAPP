...
| [[Components/CollectionView\|CollectionView]] | `Handlers/CollectionView/` | android-real (Win mux::ListView w/ SelectionMode=None/Single/Multiple + Linux GtkListBox w/ matching GtkSelectionMode + Android android.widget.ListView w/ setChoiceMode mapping; items_source binds via wrap-platform-recycler per ADR-0020. Multi-select event round-trip wired on all 3 platforms — Win iterates SelectedItems via Items.IndexOf, Linux walks gtk_list_box_get_selected_rows on "selected-rows-changed", Android calls getCheckedItemPositions() from item_click_router after each multi-mode tap. Horizontal/grid layouts + item_template tied to the cell tree are still follow-ups.) |
