...
|---|---|---|
| [[Components/AbsoluteLayout\|AbsoluteLayout]] | `Controls/Layout/AbsoluteLayout/` | mock + **windows-real + linux-real + android-real (compile-verified)** (surface + mock handler + 13 tests; per-child `layout_bounds` rect + `layout_flags` proportional bitmask attached store mirroring Grid. **Windows `mux::Canvas` (SetLeft/SetTop+size, proportional flags vs ActualWidth/Height) links into `mpapp-handlers-windows` (MSVC); Linux `GtkFixed`; Android `FrameLayout`+LayoutParams (arm64+x86_64).** on-device runtime + macOS/iOS are follow-ups) |
...
| [[Components/FlexLayout\|FlexLayout]] | `Controls/Layout/FlexLayout/` | mock + **windows-real + linux-real + android-real (compile-verified)** (surface + mock handler + 11 tests; container direction/wrap/justify/align_items/align_content/position + per-child order/grow/shrink/basis/align_self. **A real CSS-flexbox solver `layout/flex_arrange.hpp` (170-assertion test) now exists; the Windows handler hosts children in a `Canvas` and drives `flex_arrange` to real pixels (SetLeft/SetTop+size, re-solving on SizeChanged), MSVC-linked.** Linux `GtkBox` / Android `LinearLayout` v1 maps link in too — wiring them to `flex_arrange` + on-device runtime are follow-ups) |
