...
- Usage at the component layer: every surface header in [`include/mpapp/internal/`](../../include/mpapp/internal/) follows the `Observable<T> property{}` pattern — e.g. [`internal/basic_button.hpp`](../../include/mpapp/internal/basic_button.hpp)'s `Observable<std::string> text{}`. The user-facing wrapper at [`include/mpapp/button.hpp`](../../include/mpapp/button.hpp) inherits the surface publicly so every Observable is visible on `mpapp::button` itself (see [[ADR-0024-wrapper-component-pattern]] for the layering).
- Handler integration: [`include/mpapp/handlers/mock/button_handler.hpp`](../../include/mpapp/handlers/mock/button_handler.hpp) — `map_text(basic_button&)` subscribes to `text.changed` and threads the new value into either the recording (mock) or the native widget (real handlers).
