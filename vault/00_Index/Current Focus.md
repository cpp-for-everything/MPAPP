...
1. **ADR-0015 follow-ups: Skia backend + ShapeView/GraphicsView migration.** Cairo backend is now real on Linux, Windows (via vcpkg), and Android (via vcpkg + NDK; minSdk bumped to 28 for bionic libiconv). **GraphicsView migration is complete on all 3 platforms** — [[T-0029]] phases 1+2 landed pixel readback on the abstract canvas API, a `drawable` callback Observable on `graphics_view`, and all three platform handlers wired against the facade: Linux blits through GTK's cairo context; Windows uses `muxc::Image` + `WriteableBitmap` (BGRA8 matches the facade format — single memcpy per row); Android uses `ImageView` + `Bitmap.ARGB_8888` (with a per-pixel B↔R swap because Android's bitmap byte order is RGBA in memory, not BGRA). **Skia backend scaffolding + impl** landed in [[T-0030]] — opt-in via `-DMPAPP_GRAPHICS_BACKEND=skia` once user runs `vcpkg install skia:<triplet>` (~30 min from source, cached after); falls back to stub cleanly when Skia isn't installed. ShapeView migration is still outstanding (replaces existing real handlers' per-platform shape primitives with facade calls — visual-regression-risky so deferred until screenshots are available).
