...
>
> **The app is a gap detector.** Building a real screen surfaced the next priorities. Closed so far: ~~`label` font/color/weight~~ (**[[T-0061-label-typography]]** — `font_size`/`font_bold`/`font_family`/`text_color` real on every platform; УИСС headings/titles styled in TU navy); ~~Плащания `picker`~~ (real `mpapp::picker` dropdown). Still open: data tables → `CollectionView`/`TableView` typed items; TU logo → RFC-0004 image loaders; RFC-0005 styles for app-wide theming; plus the broader goal subsystems (gestures real Win/Android, Essentials backends, VSM input-routing, CarouselView real, real event loops, hot-reload). The app tells us what to harden next.
