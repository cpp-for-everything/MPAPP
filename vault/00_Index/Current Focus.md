...
1. **ADR-0015 follow-ups: Skia backend + ShapeView/GraphicsView migration.** Cairo backend is now real on Linux, Windows (via vcpkg), and Android (via vcpkg + NDK; minSdk bumped to 28 for bionic libiconv). **GraphicsView migration is complete on all 3 platforms** — [[T-0029]] phases 1+2 landed the abstract canvas pixel-readback API, a `drawable` callback Observable on `graphics_view`, and all three platform handlers wired against the facade. **ShapeView migration is complete on all 3 platforms** — [[T-0031]] phases 1+2 landed the shared `detail::graphics::render_shape_view` helper plus per-platform real handlers all routing through it (Linux: cairo source-surface blit; Windows: `muxc::Image` + `WriteableBitmap`; Android: `ImageView` + `Bitmap.ARGB_8888` with B↔R swap). `MppShapeView.java` deleted. **Skia backend scaffolding + impl** landed in [[T-0030]] — opt-in via `-DMPAPP_GRAPHICS_BACKEND=skia` once user runs `vcpkg install skia:<triplet>` (currently building in background); once installed, swapping backends swaps rendering on every platform at once thanks to the migrations above. One open follow-up: **per-platform GUI screenshot evidence** that all three T-0029/T-0031 migrations preserve observable rendering — canvas-pipeline PNGs already landed; native-widget capture deferred to a user-idle window.
