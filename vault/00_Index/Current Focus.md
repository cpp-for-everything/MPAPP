...
1. **ADR-0015 follow-ups: Skia backend + ShapeView/GraphicsView migration.** Cairo backend is now real on Linux, Windows (via vcpkg), and Android (via vcpkg + NDK; minSdk bumped to 28 for bionic libiconv). **GraphicsView migration is complete on all 3 platforms** — [[T-0029]] phases 1+2 landed pixel readback on the abstract canvas API, a `drawable` callback Observable on `graphics_view`, and all three platform handlers wired against the facade. **Skia backend scaffolding + impl** landed in [[T-0030]] — opt-in via `-DMPAPP_GRAPHICS_BACKEND=skia` once user runs `vcpkg install skia:<triplet>`. **ShapeView migration phase 1** landed in [[T-0031]] — shared `detail::graphics::render_shape_view` helper consolidates per-kind dispatch + hex color parsing into one source of truth; Linux real handler now routes through it (Windows + Android still use their original native primitives, T-0031 phase 2). One open follow-up: **per-platform screenshot evidence** that all three T-0029/T-0031 migrations preserve observable rendering — deferred to a user-idle window.
