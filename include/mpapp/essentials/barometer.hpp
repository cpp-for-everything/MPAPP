...
//
// `mpapp::barometer` — atmospheric-pressure sensor. Counterpart to MAUI
// Essentials `Barometer`. Readings are a scalar double (hPa / millibar).
// Abstract interface + an in-memory mock whose monitoring state is
// test-controllable and that emits readings via `reading_changed` only
// while monitoring is active. Real per-platform backends implement the same
// interface and are injected via the DI container (RFC-0011). No macros;
// header-only.

#ifndef MPAPP_ESSENTIALS_BAROMETER_HPP
#define MPAPP_ESSENTIALS_BAROMETER_HPP
...
public:
    ...
    // Fires with the latest pressure reading in hPa whenever a new sample
    // arrives (only while monitoring).
...
// * push_reading(hpa) emits reading_changed only if is_monitoring(); the
//   call is silently ignored otherwise.
// * last_speed() returns the sensor_speed passed to the most recent start().
class mock_barometer final : public barometer {
public:
    // ---- barometer interface ----------------------------------------------
    ...
    // Inject a pressure reading (hPa). Emits reading_changed only while
    // monitoring.
    void push_reading(double hpa) {
        if (!monitoring_) {
            return;
        }
        reading_changed.emit(hpa);
    }
...
#endif // MPAPP_ESSENTIALS_BAROMETER_HPP
