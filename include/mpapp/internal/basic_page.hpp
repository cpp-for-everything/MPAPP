...
#ifndef MPAPP_INTERNAL_BASIC_PAGE_HPP
#define MPAPP_INTERNAL_BASIC_PAGE_HPP
...
class basic_page : public view {
public:
    basic_page() = default;
    ~basic_page() override = default;

    basic_page(const basic_page&)            = delete;
    basic_page& operator=(const basic_page&) = delete;
    basic_page(basic_page&&)                 = delete;
    basic_page& operator=(basic_page&&)      = delete;
    ...
    // ----- Lifecycle signals ---------------------------------------------
    //
    // `shell` fires `navigated_to` on the basic_page that becomes current_content
    // after a successful `go_to(uri)` (or typed go_to). It fires
    // `navigated_from` on the previously-current basic_page just before swapping
...
#endif // MPAPP_INTERNAL_BASIC_PAGE_HPP
