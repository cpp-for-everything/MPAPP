...
#ifndef MPAPP_INTERNAL_BASIC_HYBRID_WEB_VIEW_HPP
#define MPAPP_INTERNAL_BASIC_HYBRID_WEB_VIEW_HPP
...
class basic_hybrid_web_view : public internal::basic_web_view {
public:
    basic_hybrid_web_view() = default;
    ~basic_hybrid_web_view() override = default;

    basic_hybrid_web_view(const basic_hybrid_web_view&)            = delete;
    basic_hybrid_web_view& operator=(const basic_hybrid_web_view&) = delete;
    basic_hybrid_web_view(basic_hybrid_web_view&&)                 = delete;
    basic_hybrid_web_view& operator=(basic_hybrid_web_view&&)      = delete;

    // ----- JS bridge surface --------------------------------------------

    // Bound JS namespace; the real handler exposes this as basic_window[hybrid_namespace] on the JS side.
    Observable<std::string> hybrid_namespace{"mpapp"};

    // HTML content for the embedded basic_page. The real per-platform
    // handlers wire this to webkit_web_view_load_html / WebView2
    // NavigateToString / Android WebView.loadDataWithBaseURL. Setting
    // an empty string clears the basic_page; setting non-empty triggers a
    ...
    // mpapp::basic_web_view alongside this control.
    ...
    // for the basic_hybrid_web_view's lifetime. Returns a reference so the
    ...
    // arrives, the callback leaks until the basic_hybrid_web_view is
    // destroyed.
    //
    ...
    //
    // Usage:
    //   mpapp::task<int> add_via_bridge(basic_hybrid_web_view& wv) {
...
#endif // MPAPP_INTERNAL_BASIC_HYBRID_WEB_VIEW_HPP
