...
#ifndef MPAPP_INTERNAL_BASIC_NAVIGATION_PAGE_HPP
#define MPAPP_INTERNAL_BASIC_NAVIGATION_PAGE_HPP
...
class basic_navigation_page : public internal::basic_page {
public:
    basic_navigation_page() = default;
    explicit basic_navigation_page(basic_page* root) {
        if (root != nullptr) stack_.push(root);
        sync_observables();
    }
    ~basic_navigation_page() override = default;

    basic_navigation_page(const basic_navigation_page&)            = delete;
    basic_navigation_page& operator=(const basic_navigation_page&) = delete;
    basic_navigation_page(basic_navigation_page&&)                 = delete;
    basic_navigation_page& operator=(basic_navigation_page&&)      = delete;

    // ----- Read-only stack state ---------------------------------------

    Observable<basic_page*>       current_page{nullptr};
    Observable<basic_page*>       root_page{nullptr};
    Observable<std::size_t> stack_depth{0};

    // ----- Sync mutators (ADR-0014 primitive) --------------------------

    void push(basic_page* p) {
        stack_.push(p);
        sync_observables();
    }
    ...
    basic_page* pop() {
        view* v = stack_.pop();
        sync_observables();
        return static_cast<basic_page*>(v);
    }
    ...
    void insert_page_before(basic_page* before, basic_page* p) {
        stack_.insert_before(before, p);
        sync_observables();
    }

    void remove_page(basic_page* p) {
        stack_.remove(p);
        sync_observables();
    }
    ...
    [[nodiscard]] task<void> push_async(basic_page* p) {
        push(p);
        co_return;
    }

    [[nodiscard]] task<basic_page*> pop_async() {
        basic_page* popped = pop();
        co_return popped;
    }
    ...
    // the child basic_page. The navigation host reads these when it renders the
    // bar chrome for the current basic_page.
    ...
    void set_has_back_button(basic_page& p, bool v) {
        stack_.set_attached(p, has_back_button_key, v);
    }
    bool get_has_back_button(const basic_page& p) const {
        return stack_.get_attached(p, has_back_button_key, true);
    }

    void set_has_navigation_bar(basic_page& p, bool v) {
        stack_.set_attached(p, has_navigation_bar_key, v);
    }
    bool get_has_navigation_bar(const basic_page& p) const {
        return stack_.get_attached(p, has_navigation_bar_key, true);
    }

    void set_back_button_title(basic_page& p, std::string_view t) {
        stack_.set_attached(p, back_button_title_key, std::string(t));
    }
    std::string get_back_button_title(const basic_page& p) const {
...
private:
    void sync_observables() {
        basic_page* root = static_cast<basic_page*>(stack_.root());
        basic_page* top  = static_cast<basic_page*>(stack_.top());
...
#endif // MPAPP_INTERNAL_BASIC_NAVIGATION_PAGE_HPP
