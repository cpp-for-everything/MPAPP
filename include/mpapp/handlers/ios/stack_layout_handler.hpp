...
// Part of MPAPP. T-0011 — UIKit basic_stack_layout handler. Wraps UIStackView.

#ifndef MPAPP_HANDLERS_IOS_STACK_LAYOUT_HANDLER_HPP
#define MPAPP_HANDLERS_IOS_STACK_LAYOUT_HANDLER_HPP
...
template <>
class stack_layout_handler<platform::ios> {
public:
...
private:
    ...
    struct orient_cb_t   { stack_layout_handler<platform::ios>* self; void operator()(const orientation& v) const { self->apply_orientation(v); } };
    struct spacing_cb_t  { stack_layout_handler<platform::ios>* self; void operator()(const double& v) const { self->apply_spacing(v); } };
    struct padding_cb_t  { stack_layout_handler<platform::ios>* self; void operator()(const thickness& v) const { self->apply_padding(v); } };
    struct h_align_cb_t  { stack_layout_handler<platform::ios>* self; void operator()(const h_align& v) const { self->apply_horizontal_alignment(v); } };
    struct v_align_cb_t  { stack_layout_handler<platform::ios>* self; void operator()(const v_align& v) const { self->apply_vertical_alignment(v); } };
...
#endif // __APPLE__
#endif // MPAPP_HANDLERS_IOS_STACK_LAYOUT_HANDLER_HPP
