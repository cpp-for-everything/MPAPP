...
// **deprecated** `frame` control. Kept for XAML migration parity.
//
// `mpapp::frame` carries `[[deprecated]]`, so naming it inside this
...
// its own use of `frame` is intentional. Internal compiler pragmas are
...
public:
    ...
    void map_border_color(frame& f)   { bind("border_color",   f.border_color,   binding_border_color_); }
    void map_has_shadow(frame& f)     { bind("has_shadow",     f.has_shadow,     binding_has_shadow_); }
    void map_corner_radius(frame& f)  { bind("corner_radius",  f.corner_radius,  binding_corner_radius_); }
    void map_padding(frame& f)        { bind("padding",        f.padding,        binding_padding_); }
