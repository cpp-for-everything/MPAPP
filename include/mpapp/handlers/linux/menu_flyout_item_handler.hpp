...
// Part of MPAPP. GTK4 `basic_menu_flyout_item` handler — wraps a
// `GtkButton`. `text` flows through to the basic_button's basic_label;
// `is_enabled` toggles `gtk_widget_set_sensitive`. The native
// basic_button's `clicked` signal fires the cross-platform `clicked`
// signal.
...
void map_gestures(basic_menu_flyout_item& x);
