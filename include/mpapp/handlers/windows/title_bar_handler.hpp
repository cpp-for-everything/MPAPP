// SPDX-License-Identifier: Apache-2.0
// Part of MPAPP. WinUI 3 basic_title_bar handler — wraps
// `mux::Controls::TitleBar` (WinUI 3 1.5+). Surfaces the `Title` and
// `Subtitle` text slots.

#ifndef MPAPP_HANDLERS_WINDOWS_TITLE_BAR_HANDLER_HPP
#define MPAPP_HANDLERS_WINDOWS_TITLE_BAR_HANDLER_HPP

#include "../../platform.hpp"
#include "../../signal.hpp"
#include "../../internal/basic_title_bar.hpp"

#if defined(_WIN32)

#include <string>

#include <winrt/Microsoft.UI.Xaml.Controls.h>

namespace mpapp::internal {

template <>
class title_bar_handler<platform::windows> {
public:
    title_bar_handler();
    ~title_bar_handler();
    title_bar_handler(const title_bar_handler&)            = delete;
    title_bar_handler& operator=(const title_bar_handler&) = delete;
    title_bar_handler(title_bar_handler&&)                 = delete;
    title_bar_handler& operator=(title_bar_handler&&)      = delete;

    void map_title(basic_title_bar& t);
    void map_subtitle(basic_title_bar& t);

    winrt::Microsoft::UI::Xaml::Controls::TitleBar&       native() noexcept       { return native_; }
    const winrt::Microsoft::UI::Xaml::Controls::TitleBar& native() const noexcept { return native_; }

private:
    void apply_title(const std::string& v);
    void apply_subtitle(const std::string& v);

    struct title_cb_t    { title_bar_handler<platform::windows>* self; void operator()(const std::string& v) const { self->apply_title(v); } };
    struct subtitle_cb_t { title_bar_handler<platform::windows>* self; void operator()(const std::string& v) const { self->apply_subtitle(v); } };

    winrt::Microsoft::UI::Xaml::Controls::TitleBar native_{nullptr};

    title_cb_t                       title_cb_{this};
    subtitle_cb_t                    subtitle_cb_{this};
    signal_slot<const std::string&>  title_slot_{};
    signal_slot<const std::string&>  subtitle_slot_{};
};

} // namespace mpapp::internal
#endif // _WIN32
#endif // MPAPP_HANDLERS_WINDOWS_TITLE_BAR_HANDLER_HPP
