...
#define MPAPP_HANDLERS_WINDOWS_FRAME_HANDLER_HPP
...
private:
    ...
    struct content_cb_t       { frame_handler<platform::windows>* self; void operator()(const std::shared_ptr<view>& v) const { self->apply_content(v); } };
    struct border_color_cb_t  { frame_handler<platform::windows>* self; void operator()(const color& c) const { self->apply_border_color(c); } };
    struct has_shadow_cb_t    { frame_handler<platform::windows>* self; void operator()(bool b) const { self->apply_has_shadow(b); } };
    struct corner_radius_cb_t { frame_handler<platform::windows>* self; void operator()(float r) const { self->apply_corner_radius(r); } };
    struct padding_cb_t       { frame_handler<platform::windows>* self; void operator()(const thickness& t) const { self->apply_padding(t); } };
...
#endif // MPAPP_HANDLERS_WINDOWS_FRAME_HANDLER_HPP
