...
#define MPAPP_HANDLERS_ANDROID_STACK_LAYOUT_HANDLER_HPP
...
private:
    ...
    struct orient_cb_t   { stack_layout_handler<platform::android>* self; void operator()(const orientation& v) const { self->apply_orientation(v); } };
    struct spacing_cb_t  { stack_layout_handler<platform::android>* self; void operator()(const double& v) const { self->apply_spacing(v); } };
    struct padding_cb_t  { stack_layout_handler<platform::android>* self; void operator()(const thickness& v) const { self->apply_padding(v); } };
    struct h_align_cb_t  { stack_layout_handler<platform::android>* self; void operator()(const h_align& v) const { self->apply_horizontal_alignment(v); } };
