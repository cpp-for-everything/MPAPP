...
// Part of MPAPP. T-0026 — WebView demo (Linux/GTK4 + WebKitGTK 6.x).
//
// Visible exercise of `mpapp::web_view`:
//   * `html_source` Observable — set to a small HTML page so the
//     demo doesn't depend on network access.
//   * `is_loading` / `navigated` — observed via a status label that
//     reports loading state + last-navigated URL (the WebKit/WebView2
//     embeds report about:blank for inline HTML loads).
//   * "Toggle content" button — swaps between two embedded HTML
//     pages to exercise html_source.changed end-to-end.
...
public:
    void on_launch() override {
        ...
        window_.title  = "MPAPP T-0026 - WebView Demo (GTK4 + WebKitGTK)";
        window_.width  = 720;
        window_.height = 520;
...
private:
    ...
    struct nav_cb_t {
        webview_demo_app* self;
        void operator()(const std::string& url, bool /*success*/) const {
    ...
    struct loading_cb_t {
        webview_demo_app* self;
        void operator()(bool /*v*/) const { self->refresh_status(); }
    };
