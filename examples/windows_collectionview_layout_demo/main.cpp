...
// Part of MPAPP. T-0028 closure evidence — Windows WinUI 3 demo
// showing all four collection_layout values rendered side by side.
// One screenshot captures the full matrix:
//
//   * vertical_list   — single column, items stack downward
//   * horizontal_list — single row,   items extend rightward
//   * vertical_grid   — multi-column, items wrap downward
//   * horizontal_grid — multi-row,    items wrap rightward
//
// Each section has a label header so the screenshot is self-
// documenting.
...
struct cv_section {
    mpapp::label                       header{};
    mpapp::label_handler<wp>           header_handler{};
    mpapp::collection_view             cv{};
    mpapp::collection_view_handler<wp> cv_handler{};
};
